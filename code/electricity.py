...
##
## Desarrollado por Diego Maroto ( BilbaoMakers ) actualizado con Elastic y Docker por Daniel Meijer (danielmeijer)
##
...
# Crear index en Elasticsearch
es = Elasticsearch(host='elastic', port=None)
es.indices.create(index='electricity', ignore=400)

## PREPARAR LA LLAMADA A LA API
url = 'https://api.esios.ree.es/indicators/1001'
headers = {'Accept':'application/json; application/vnd.esios-api-v2+json','Content-Type':'application/json','Host':'api.esios.ree.es','Authorization':'Token token=\"'+ os.environ['token'] +'\"'}
...
while True:
  ...
  if response.status_code == 200:
    ...
    for t_valor in valores_geoid:
      ...
      res = es.index(index='electricity', document=t_valor, op_type='create')
      print(res['result']) 
